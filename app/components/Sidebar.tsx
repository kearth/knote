...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside className={`sidebar ${isCollapsed ? 'collapsed' : ''}`} id="sidebar" style={{ display: 'flex', flexDirection: 'column', height: '100%', width: isCollapsed ? '60px' : '250px', transition: 'width 0.3s ease' }}>
      <div className="sidebar-header">
        <div className="logo" style={{ display: isCollapsed ? 'none' : 'block', padding: '0px 10px' }}>
          <img src="/images/logo.jpg" width="72" height="72" alt="logo" />
        </div>
      </div>

      <nav className="nav-section" style={{ flexGrow: 1, overflowY: 'auto', padding: isCollapsed ? '10px 0' : '0px 10px' }}>
        <SearchContainer placeholder="搜索笔记..." isCollapsed={isCollapsed} />

        <NavItem
          icon={
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke-width="2">
        ...
        <CollapsibleSection
          title="Pinned"
          icon={
            <svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" height="18px" viewBox="0 0 24 24" width="18px" fill="#1f1f1f">
        ...
        <CollapsibleSection
          title="History"
          icon={
            <svg xmlns="http://www.w3.org/2000/svg" height="18px" viewBox="0 0 24 24" width="18px" fill="#1f1f1f">
              <path d="M0 0h24v24H0V0z" fill="none" />
